...
<div class="max-w-2xl mx-auto bg-white p-8 rounded-xl shadow-sm border border-slate-100">
    ...
    <div class="flex justify-between items-center mb-6 border-b border-slate-100 pb-4">
        <div>
            ...
            <p class="text-xs text-slate-400 font-mono mt-1">ID de Control: #{{ $appointment->id }}</p>
        </div>
        <span class="px-3 py-1 rounded-full text-xs font-bold uppercase {{ $appointment->status === 'pendiente' ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-700' }}">
            {{ $appointment->status }}
        </span>
    </div>
    ...
    <div class="space-y-6">
        <div class="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-lg border border-slate-100">
            <div>
                ...
                <span class="text-sm font-medium text-slate-700 font-mono">{{ $appointment->appointment_date }}</span>
            </div>
            <div>
                <span class="block text-xs font-semibold text-slate-400 uppercase tracking-wider">Hora de Atención</span>
                <span class="text-sm font-medium text-slate-700 font-mono">{{ $appointment->appointment_time }}</span>
            </div>
        </div>
        ...
        <div class="space-y-3">
            <div>
                ...
                <span class="text-base font-semibold text-slate-800">Dr. {{ $appointment->doctor?->name }}</span>
            </div>
            <div>
            ...
            <div>
                ...
                <span class="text-sm font-medium text-slate-600">{{ $appointment->patient?->name }}</span>
            </div>
        </div>
        ...
        <div class="border-t border-slate-100 pt-4">
            ...
            <p class="text-sm text-slate-600 bg-slate-50 p-3 rounded-lg italic border border-slate-100 leading-relaxed whitespace-pre-line">
                {{ $appointment->symptoms ?? 'No se describieron síntomas específicos para esta consulta.' }}
            </p>
        </div>

        <div class="text-[11px] text-slate-400 flex justify-between pt-2 border-t border-slate-100">
            <span>Registrada el: {{ $appointment->created_at->format('d/m/Y g:i a') }}</span>
            <span>Última actualización: {{ $appointment->updated_at->format('d/m/Y g:i a') }}</span>
        </div>

        <div class="flex justify-end pt-4">
            <a href="{{ route('appointments.index') }}" class="bg-slate-800 hover:bg-slate-900 text-white px-5 py-2 rounded-lg text-sm font-semibold shadow-sm transition">
